...
function calculateStatistics() {
    ...
    var nest = d3.nest().key(function (d) {
        ...
        _this.config.value_cols.forEach(function (value_col) {
            ...
            summary[value_col.col] = crfsNoDenominator.indexOf(value_col.col) > -1 ? summary.nForms ? count / summary.nForms : 'N/A' : crfsDenominator.indexOf(value_col.col) > -1 ? summary['n' + value_col.denominator] ? count / summary['n' + value_col.denominator] : 'N/A' : queries.indexOf(value_col.col) > -1 ? count : console.log("Missed one: " + value_col.col);
        });
...
function defineStyles() {
    ...
    '#chm-legend-container {' + ('    width: ' + (100 - firstColumnWidth) + '%;') + '    float: right;' + '    display: inline-block;' + '    height: 100%;' + '}', '.chm-legend {' + '    padding-top: 17px;' + '    display: inline-block;' + '}', '.chm-legend > * {' + '}', '#chm-crf-legend {' + '    float: left;' + '    width: 74.9%;' + '}', '#chm-query-legend {' + '    float: right;' + '    width: 24.9%;' + '}', '.chm-legend-title {' + '    font-size: 20px;' + '    font-weight: bold;' + '}', '#chm-query-legend .chm-legend-title {' + '    text-align: right;' + '}', '.chm-legend-div {' + '    display: inline-block;' + '    height: 20px;' + '    text-align: center;' + '    font-weight: bold;' + '    font-size: 14px;' + '}', '#chm-crf-legend .chm-legend-div {' + '    width: 20%;' + '}', '#chm-query-legend .chm-legend-div {' + '    width: 20%;' + '}',
...
function rendererSettings() {
    return {
        ...
        value_cols: [{ col: 'is_partial_entry', type: 'crfs', label: 'Entered' }, { col: 'verified', type: 'crfs', denominator: 'needs_verification', label: 'Source Data Verified' }, { col: 'ready_for_freeze', type: 'crfs', label: 'Ready for Freeze' }, { col: 'is_frozen', type: 'crfs', label: 'Frozen' }, { col: 'is_signed', type: 'crfs', denominator: 'needs_signature', label: 'Signed' }, { col: 'is_locked', type: 'crfs', label: 'Locked' }, { col: 'open_query_ct', type: 'queries', label: 'Open' }, { col: 'answer_query_ct', type: 'queries', label: 'Answered' }],
...
function syncSettings(settings) {
    ...
    settings.nestings.forEach(function (nesting) {
        nesting.value_col = nesting.value_col || settings[settingsCols.find(function (settingsCol) {
...
function customizeRows() {
    ...
    this.rows.classed('chm-table-row', true).classed('chm-table-row--expandable', function (d) {
...
function customizeCells() {
    ...
    this.cells.attr('class', function (d) {
        ...
        return d.col === 'id' ? d.text.split('  |')[d.text.split('  |').length - 1] : d.col.indexOf('query') < 0 ? d.text === 'N/A' ? d.text : String(Math.floor(d.text * 100)) + '%' : d.text;
    });
}
...
function addInfoBubbles() {
    ...
    var infoMapping = {
        ...
        ready_for_freeze: 'All required cleaning is complete (e.g. SDV, queries resolved) and data are ready to be frozen in EDC.',
    ...
    chart.wrap.select('tr').selectAll('th:not(.id)').data(chart.initial_config.value_cols).append('span').html(' &#9432').attr('title', function (d) {
...
function addRowDisplayToggle() {
    ...
    var expandable_rows = this.rows.data(chart.data.raw).filter(function (d) {
    ...
    expandable_rows.on('click', function (d) {
        ...
        if (collapsed) {
            rowChildren.classed('chm-hidden chm-table-row--collapsed', true).classed('chm-table-row--expanded', false);
        } else {
...
function csv() {
    ...
    this.export.data.forEach(function (d) {
        ...
        var row = _this.export.cols.map(function (col, i) {
            var value = _this.config.value_cols.indexOf(col) > -1 && col.indexOf('query') < 0 && ['N/A', ''].indexOf(d[col]) < 0 ? Math.floor(d[col] * 100) : d[col];
...
function xlsx() {
    ...
    var arrayOfArrays = this.export.data.map(function (d) {
        return _this.export.cols.map(function (col) {
            return _this.config.value_cols.indexOf(col) > -1 && col.indexOf('query') < 0 && ['N/A', ''].indexOf(d[col]) < 0 ? Math.floor(d[col] * 100) / 100 : d[col];
        });
...
function checkRequiredVariables() {
    ...
    }), this.settings.synced.value_cols, this.settings.synced.filter_cols])).values();
    var missingVariables = requiredVariables.filter(function (variable) {
        return _this.data.variables.indexOf(variable.split(' (')[0]) < 0;
    });
    if (missingVariables.length) alert('The data are missing ' + (missingVariables.length === 1 ? 'this variable' : 'these variables') + ': ' + missingVariables.join(', ') + '.');
}
