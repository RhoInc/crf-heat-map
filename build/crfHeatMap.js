...
function summarizeData() {
    ...
    this.config.id_cols.forEach(function (id_col, i) {
        ...
        _this.data.initial_filtered.forEach(function (d) {
            ...
            d.nest_1 = _this.config.id_cols // add nest to data
            .slice(0, i + 1).map(function (id_col1) {
                return d[id_col1];
            })[0];
            d.nest_2 = _this.config.id_cols.slice(0, i + 1).map(function (id_col1) {
                return d[id_col1];
            })[1];
        });
...
function customizeCheckboxes() {
    ...
    this.controls.wrap.selectAll('.control-group').filter(function (d) {
        ...
        var expandable_rows = context.rows.filter(function (d) {
...
function addRowDisplayToggle() {
    ...
    expandable_rows.on('click', function (d) {
        ...
        function iterativeCollapse(d) {
            ...
            if (d.children) {
                d.children.classed('chm-hidden chm-table-row--collapsed', true).classed('chm-table-row--expanded', false);
