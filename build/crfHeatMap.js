...
function calculateStatistics() {
    ...
    var nest = d3.nest().key(function (d) {
        ...
        _this.config.value_cols.forEach(function (value_col) {
            ...
            summary[value_col] = ['is_partial_entry', 'ready_for_freeze', 'is_frozen', 'is_locked'].indexOf(value_col) > -1 ? summary.nForms ? count / summary.nForms : 'N/A' : ['verified'].indexOf(value_col) > -1 ? summary.nNeedsVerification ? count / summary.nNeedsVerification : 'N/A' : ['is_signed'].indexOf(value_col) > -1 ? summary.nNeedsSignature ? count / summary.nNeedsSignature : 'N/A' : ['open_query_ct', 'answer_query_ct'].indexOf(value_col) > -1 ? count : console.log('Missed one: ' + value_col);
        });
...
function defineStyles() {
    ...
    '.wc-table table tbody tr:hover td {' + '    border-bottom: 1px solid black;' + '}', '.wc-table table tbody tr:hover td:first-child {' + '    border-left: 1px solid black;' + '}',
...
function rendererSettings() {
    return {
        ...
        value_cols: ['is_partial_entry', 'verified', 'ready_for_freeze', 'is_frozen', 'is_signed', 'is_locked', 'open_query_ct', 'answer_query_ct'],
...
function customizeCells() {
    ...
    d3.select('th.answer_query_ct').attr('title', 'Site has closed issue, but DM needs to close or requery.');
...
function addRowDisplayToggle() {
    ...
    var childLookup = childNest.rollup(function (d) {
        return d;
    }).map(chart.data.raw); //filtered_data?
    ...
    expandable_rows.on('click', function (d) {
        ...
        function findChildren(d) {
            ...
            if (config.id_cols.length == 2) {
                children = childLookup[d[config.id_cols[0]]].map(function (d) {
                    return d[config.id_cols[1]] ? rows[d['index']] : null;
                });
            } else if (config.id_cols.length == 3) {
                if (d[config.id_cols[1]]) {
                    children = childLookup[d[config.id_cols[0]]][d[config.id_cols[1]]].map(function (d) {
                        return d[config.id_cols[2]] ? rows[d['index']] : null;
                    });
                } else {
                    children = [];
        ...
        if (collapsed) {
            ...
            d3.selectAll(children).classed('chm-hidden chm-table-row--collapsed', true).classed('chm-table-row--expanded', false);
        } else {
            findChildren(d);
...
function csv() {
    ...
    this.export.data.forEach(function (d) {
        ...
        var row = _this.export.cols.map(function (col, i) {
            var value = _this.config.value_cols.indexOf(col) > -1 && col.indexOf('query') < 0 && ['N/A', ''].indexOf(d[col]) < 0 ? Math.round(d[col] * 100) : d[col];
...
function xlsx() {
    ...
    var arrayOfArrays = this.export.data.map(function (d) {
        return _this.export.cols.map(function (col) {
            return _this.config.value_cols.indexOf(col) > -1 && col.indexOf('query') < 0 && ['N/A', ''].indexOf(d[col]) < 0 ? d[col] : d[col];
        });
