...
function addRowDisplayToggle() {
    ...
    var b = childNest.rollup(function (d) {
        return d;
    ...
    expandable_rows.on('click', function (d) {
        ...
        function iterativeCollapse(d) {
            ...
            child = b[d[config.id_cols[0]]].map(function (d) {
                return d[config.id_cols[1]] ? rows[d['index']] : null;
            }); //grab the appropriate row from rows using index
            child.shift();
            console.log(child);
            if (child) {
                d3.selectAll(child).classed('chm-hidden chm-table-row--collapsed', true).classed('chm-table-row--expanded', false);
