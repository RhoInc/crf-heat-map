...
function defineStyles() {
    ...
    '.range-slider-container {' + '    position: relative;' + '    width: 100%;' + '    height: 25px;' + '}', '.range-slider {' + '    width: 100%;' + '    position: absolute;' + '    height: 21px;' + '    top: 2px;' + '    overflow: hidden;' + '    outline: none;' + '}', '.range-slider::-webkit-slider-thumb {' + '    pointer-events: all;' + '    position: relative;' + '    z-index: 1;' + '    outline: 0;' + '}', '.range-slider::-moz-range-thumb {' + '    pointer-events: all;' + '    position: relative;' + '    z-index: 10;' + '    -moz-appearance: none;' + '    width: 9px;' + '}', '.range-slider::-moz-range-track {' + '    position: relative;' + '    z-index: -1;' + '    background-color: rgba(0, 0, 0, 1);' + '    border: 0;' + '}', '.range-slider::-moz-range-track {' + '    -moz-appearance: none;' + '    background: none transparent;' + '    border: 0;' + '}', '.range-slider::-moz-focus-outer {' + '    border: 0;' + '}',
...
function addSliders(th, d) {
    var sliders = {
        lower: 0,
        upper: d.indexOf('query') < 0 ? 100 : d3.max(this.data.raw, function (di) {
            return di[d];
        })
    };

    //elements
    sliders.container = d3.select(th).append('div').classed('range-slider-container', true);
    sliders.lowerInput = sliders.container.append('input').classed('range-slider', true).attr({
        value: sliders.lower,
        min: sliders.lower,
        max: sliders.upper,
        step: 1,
    ...
    sliders.upperInput = sliders.container.append('input').classed('range-slider', true).attr({
        value: sliders.upper,
        min: sliders.lower,
        max: sliders.upper,
        step: 1,
...
function addColumnFilters() {
    ...
    this.columnFilters = {
        ...
        variables: this.config.cols.filter(function (d) {
            return d !== 'id';
        }),
        sliders: []
    };
    this.columnFilters.headers = this.columnFilters.header.selectAll('th').data(this.config.cols).enter().append('th').filter(function (d) {
        return d !== 'id';
    }).each(function (d) {
        addSliders.call(context, this, d);
    });
    d3.merge(this.columnFilters.sliders.map(function (slider) {
        return [slider.lowerInput, slider.upperInput];
