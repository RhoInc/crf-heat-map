...
var webchartsSettings = {
    ...
    value_cols: ['is_partial_entry', 'is_verified', 'has_open_query', 'has_answered_query', 'is_frozen', 'is_signed', 'is_locked'],
    filter_cols: ['sitename', 'ready_for_freeze', 'status'],
    pagination: false,
    searchable: false,
    sortable: false,
    headers: ['ID', 'CRFs Entered', 'Source Data Verified', 'Opened Queries*', 'Answered Queries*', 'Frozen', 'Signed', 'Locked'],
    cols: null
};
...
function processData(data, settings, level) {
    ...
    longData.forEach(function (d) {
        d.nestKey = '';
        settings.id_cols.filter(function (d, i) {
            return i < level;
...
function drawLegend() {
    ...
    var heatLegend = this.wrap.insert('svg', ':first-child').classed('legend', true).attr('width', legendWidth).attr('height', legendHeight).selectAll('.legend').data(colors).enter();

    heatLegend.append('rect').style({
    ...
    d3.select("svg.legend").selectAll("text").data(topTextData).enter().append("text").text(function (d) {
        return d;
    }).each(function (d, i) {
    ...
    }).attr('y', (legendHeight - rectHeight) / 2 - 5).append('svg:tspan').attr('x', legendWidth - 80).attr('dy', -20).text("← Queries*");

    // Tick Labels for Bottom Axis (Proportions)
    var bottomTextData = ['0%-25%', '25%-50%', '50%-75%', '75%-<100%', '100%'];

    //Information for Proportions
    d3.select("svg.legend").selectAll("g").data(bottomTextData).enter().append('text').text(function (d) {
        return d;
    }).attr('x', function (d, i) {
        return rectWidth * i;
    }).attr('y', (legendHeight - rectHeight) / 2 + rectHeight + 15).append('svg:tspan').attr('x', 0).attr('dy', 20).text("Forms →");
}
...
function onDraw() {
    ...
    var colorScaleProp = d3.scale.quantile().domain([0, 1]).range(colors.slice(0, 4)); // we will want to set the dark color for 1 specifically

    //Separate Color Scale for Queries
    var colorScaleSum = d3.scale.quantile().domain([0, 32]) // so that the last section is 24 and above
    ...
    var cells = rows.selectAll('td').style('background', function (d) {
        if (d.col.includes('query')) {
            // deal with query columns first
    ...
    cells.filter(function (d) {
        return d.col != 'id';
    }).style('text-align', 'center').style('color', 'transparent').style('width', '100px').on('mouseover', function () {
        d3.select(this).style('color', function (d) {
            if (d.col.includes('query')) return +d.text > 12 ? 'black' : 'white';else return d.text > 0.5 ? 'white' : 'black';
        });
    ...
    rows.each(function (d) {
        ...
        d3.select(this).select('td').text(label).style('padding-left', function (d) {
            return d.level - 1 + 'em';
        });
    });
    ...
    var expandable_rows = rows.filter(function (d) {
        return d.level < config.id_cols.length;
    }).classed('collapsed', true).select('td').style('color', 'blue').style('text-decoration', 'underline').style('cursor', 'pointer');
    ...
    expandable_rows.on('click', function (d) {
        ...
        row.classed('collapsed', collapsed) //toggle the class
