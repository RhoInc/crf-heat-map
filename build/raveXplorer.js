...
function onDraw() {
    ...
    var colorScaleProp = d3.scale.quantile().domain([0, 1.25]) //because the first bar is only for those with 0 queries
    .range(colors);

    //Separate Color Scale for Queries
    var colorScaleSum = d3.scale.quantile().domain([0, 30]).range(colorsReversed);
