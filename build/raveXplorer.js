...
var value_columns = ['is_partial_entry', 'is_verified', 'has_open_query', 'has_answered_query', 'is_frozen', 'is_signed', 'is_locked'];
...
function onLayout() {
  ...
  *************************************/
  ...
  var idSelects = idControlWrap.selectAll('select').data([0, 1, 2, 3]).enter().append('select');

  idSelects.selectAll('option').data(idList).enter().append('option').text(function (d) {
    return d;
  ...
  **************************************/
  ...
  var heatLegend = this.wrap.insert('svg', ':first-child').classed('legend', true).attr('width', 500).attr('height', 100).selectAll('.legend').data(colors).enter();
  ...
  heatLegend.append('rect').style({
    ...
    'fill-opacity': 1
  }).attr('width', 100).attr('height', 20).attr('x', function (d, i) {
    return 100 * i;
  }).attr('y', 40);

  //Information for Proportions
  heatLegend.append('text').text(function (d, i) {
    if (i == 0) {
      return '0% of Forms';
    } else if (i == 2) {
      return '50%';
...
function onDraw() {
  ...
  var colorScaleProp = d3.scale.quantile().domain([0, 1]).range(colors);
