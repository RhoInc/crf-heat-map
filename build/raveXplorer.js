...
function defineStyles() {
    ...
    '.cell--heat {' + '    text-align: center;' + '    color: transparent;' + '    width: 100px;' + '}', '.cell--heat--level1:hover,' + '.cell--heat--level2:hover,' + '.cell--heat--level3:hover {' + '    color: black;' + '}', '.cell--heat--level4:hover,' + '.cell--heat--level5:hover {' + '    color: white;' + '}', '.cell--heat--level1 {' + '    background: #eff3ff;' + '}', '.cell--heat--level2 {' + '    background: #bdd7e7;' + '}', '.cell--heat--level3 {' + '    background: #6baed6;' + '}', '.cell--heat--level4 {' + '    background: #3182bd;' + '}', '.cell--heat--level5 {' + '    background: #08519c;' + '}'];
...
function customizeCells() {
    ...
    this.cells.attr('class', function (d) {
        ...
        if (d.col === 'id') cellClass = cellClass + ' cell--id' + ' cell--id--level' + d.text.substring(0, 1);else {
            ...
            var level = void 0;
            if (d.col.indexOf('query') > -1) level = d.text === 0 ? 5 : d.text < 9 ? 4 : d.text < 17 ? 3 : d.text < 25 ? 2 : 1;else level = d.text === 1 ? 5 : d.text > 0.75 ? 4 : d.text > 0.5 ? 3 : d.text > 0.25 ? 2 : 1;
            cellClass = cellClass + ' cell--heat--level' + level;
        }

        return cellClass;
    }).text(function (d) {
        return d.col === 'id' ? d.text.indexOf(':') > -1 ? d.text.substring(d.text.lastIndexOf(':') + 1) : d.text.substring(1) : d.col.indexOf('query') < 0 ? d3.format('%')(d.text) : d.text;
    });
}
