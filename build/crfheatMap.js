...
function drawLegend() {
    ...
    d3.select('svg.legend').append('text').text('Queries').style({
        'font-weight': 'bold',
        'font-size': '17px'
    }).attr('x', idCellWidth + heatCellWidth * 5).attr('y', legendHeight - rectHeight - 25);
}
    ...
    if (table.config.id_cols[table.config.id_cols.length - 1] === 'subjectnameoridentifier') {
        ...
        if (table.config.id_cols.length == 1) {
            this.export.data.forEach(function (subject, index, objects) {
                ...
                subject['status'] = subjectmap[subject['Nest 1: subjectnameoridentifier']]['status'];
                subject['freeze'] = subjectmap[subject['Nest 1: subjectnameoridentifier']]['freeze'];
            });
        }
        ...
        // case 2: Subject is lowest of two nests
                ...
                this.export.data.forEach(function (levelTwo, index, object) {
                    ...
                    ? object.splice(index, 1) : null;
                    if (typeof levelTwo.children != 'undefined') {
                        levelTwo.children[0].forEach(function (d) {
                            d.__data__['site'] = subjectmap[d.__data__['Nest 2: subjectnameoridentifier']]['site'];
                            d.__data__['status'] = subjectmap[d.__data__['Nest 2: subjectnameoridentifier']]['status'];
                            d.__data__['freeze'] = subjectmap[d.__data__['Nest 2: subjectnameoridentifier']]['freeze'];
                        });
                    }
                });
            }
                            ...
                            levelThree.children[0].forEach(function (levelTwo) {
                                if (typeof levelTwo.__data__.children != 'undefined') {
                                    levelTwo.__data__.children[0].forEach(function (d) {
                                        d.__data__['site'] = subjectmap[d.__data__['Nest 3: subjectnameoridentifier']]['site'];
                                        d.__data__['status'] = subjectmap[d.__data__['Nest 3: subjectnameoridentifier']]['status'];
                                        d.__data__['freeze'] = subjectmap[d.__data__['Nest 3: subjectnameoridentifier']]['freeze'];
                                    });
                                }
                            });
                        }
