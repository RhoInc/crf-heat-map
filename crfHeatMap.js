(function (global, factory) {
    ...
    function calculateStatistics() {
      ...
      var nest = d3.nest().key(function (d) {
        ...
        context.initial_config.value_cols.forEach(function (value_col) {
          ...
          summary[value_col.col + "fraction"] = crfsNoDenominator.map(function (m) {
            return m.col;
          }).indexOf(value_col.col) > -1 ? summary.nForms ? count.toString() + "/" + summary.nForms.toString() : null : crfsDenominator.map(function (m) {
            return m.col;
          }).indexOf(value_col.col) > -1 ? summary['n' + value_col.denominator] ? count.toString() + "/" + summary['n' + value_col.denominator].toString() : null : null;
          summary[value_col.col] = crfsNoDenominator.map(function (m) {
            return m.col;
          }).indexOf(value_col.col) > -1 ? summary.nForms ? Math.floor(count / summary.nForms * 100) / 100 : 'N/A' : crfsDenominator.map(function (m) {
            return m.col;
          }).indexOf(value_col.col) > -1 ? summary['n' + value_col.denominator] ? Math.floor(count / summary['n' + value_col.denominator] * 100) / 100 : 'N/A' : queries.map(function (m) {
            return m.col;
          }).indexOf(value_col.col) > -1 ? count : console.log("Missed one: ".concat(value_col.col));
        });
      ...
      console.log(nest); //Add summarized data to array of summaries.
          ...
          for (i = 0; i < context.config.key_cols.length; i++) {
            if (aIds[i] === bIds[i]) {
              ...
              if (i === formIndex && context.initial_config.form_order_col) {
                return typeof aIds[i] == 'undefined' ? -1 : parseFloat(a.form_order) < parseFloat(b.form_order) ? -1 : 1; // use visit_order_col if provided
              }

              if (i === visitIndex && context.initial_config.visit_order_col) {
                return typeof aIds[i] == 'undefined' ? -1 : parseFloat(a.visit_order) < parseFloat(b.visit_order) ? -1 : 1;
              } else {
    ...
    function summarizeData() {
      ...
      this.config.key_cols.forEach(function (id_col, i) {
        ...
        _this.data.initial_filtered.forEach(function (d) {
          d.nest_level = i;
          d.id = _this.config.key_cols.slice(0, i + 1).map(function (id_col1) {
            return d[id_col1];
          }).join('  |');
          d.parents = [];

          if (d.nest_level == 2) {
            d.parents.push(_this.config.key_cols.slice(0, 2).map(function (id_col1) {
              return d[id_col1];
            }).join('  |'));
          }

          if (d.nest_level == 1) {
            d.parents.push(_this.config.key_cols.slice(0, 1).map(function (id_col1) {
              return d[id_col1];
            }).join('  |'));
          }
        });
    ...
    function update(filter) {
      ...
      filter.lowerAnnotation.text("".concat(context.typeDict[filter.variable] == 'crfs' ? Math.round(filter.lower * 100) : filter.lower).concat(context.typeDict[filter.variable] == 'crfs' ? '%' : '')); //update upper slider and annotation

      if (reset) filter.upperSlider.attr({
        min: filter.min,
        max: filter.max
      }).property('value', filter.upper);
      filter.upperAnnotation.text("".concat(context.typeDict[filter.variable] == 'crfs' ? Math.round(filter.upper * 100) : filter.upper).concat(context.typeDict[filter.variable] == 'crfs' ? '%' : ''));
    }
    ...
    function customizeNestOptions(key_cols) {
      ...
      this.containers.main.selectAll('#chm-nest-control--3').property('disabled', key_cols.length === 1 ? true : false); // hide options that are selected in higher level nests

      this.containers.nestControls.selectAll('#chm-nest-control--3, #chm-nest-control--2').selectAll('option').style('display', function (d) {
      ...
      this.containers.main.selectAll('#chm-nest-control--2').selectAll('option').filter(function (d) {
        return d.label === 'None';
    ...
    function createNestControls() {
      ...
      this.containers.nestControls.append('span').attr('class', 'chm-control-label').text(''); //  var idNote = this.containers.nestControls.append('span').attr('class', 'span-description');
    ...
    function defineLayout() {
      this.containers = {
        main: d3.select(this.element).append('div').datum(this).classed('crf-heat-map', true).attr('id', "crf-heat-map".concat(document.querySelectorAll('.crf-heat-map').length))
      }; // display warning message to user if they are using IE

      if (isIE) {
        this.containers.main.append('p').classed('chm-ie-sucks', true).text('Internet Explorer use is not recommended with the CRF Heat Map. You are likely to experience slower loading times.');
      }
      /**-------------------------------------------------------------------------------------------\
      Left column
      \-------------------------------------------------------------------------------------------**/
    ...
    function defineStyles() {
      ...
      /****---------------------------------------------------------------------------------\
        Row 2 - Controls
      \---------------------------------------------------------------------------------****/
      '#chm-controls .wc-controls {' + '    margin-right: 10px;' + '}', '#chm-controls .control-group {' + '    width: 100%;' + '    margin: 0 0 5px 0;' + '}', '#chm-controls .control-group > * {' + '    display: block;' + '    width: auto;' + '}', '#chm-controls .wc-control-label {' + '    text-align: center;' + '}', '#chm-controls .span-description {' + '}', '#chm-controls select.changer {' + '    margin: 0 auto;' + '}', '.chm-control-grouping {' + '    display: inline-block;' + '}', '.chm-control-grouping .control-group .wc-control-label {' + '    text-align: center;' + '}', '.chm-control-grouping--label {' + '    text-align: center;' + '    width: 100%;' + '    font-size: 20px;' + '    margin-bottom: 5px;' + '}', '.chm-filters {' + '    display: flex;' + '    flex-wrap: wrap ;' + '    justify-content: space-evenly;' + '    border-bottom: 1px solid lightgray;' + '    padding-bottom: 7px;' + '}', '.chm-other-controls {' + '    margin-top: 10px;' + '    display: flex;' + '    flex-wrap: wrap;' + '    justify-content: space-evenly;' + '}', //checkboxes
      ...
      /****---------------------------------------------------------------------------------\
        Nest controls
      \---------------------------------------------------------------------------------****/
      '#chm-nest-controls {' + "    width: ".concat(firstColumnWidth, "%;") + '    height: 100%;' + '}', '.chm-nest-control {' + '    float: left;' + '    display: block;' + '    clear: left;' + "    padding-left: ".concat(paddingLeft, "px;") + '    min-width : 100px;' + '}', '.chm-nest-control.chm-hide {' + '    float: left;' + '    display: none;' + '    clear: left;' + "    padding-left: ".concat(paddingLeft, "px;") + '}', '#chm-nest-control--1 {' + '    margin-left: 0;' + '}', '#chm-nest-control--2 {' + '    margin-left: 1em;' + '}', '#chm-nest-control--3 {' + '    margin-left: 2em;' + '}',
      /****---------------------------------------------------------------------------------\
        Legend
      \---------------------------------------------------------------------------------****/
      '#chm-legend-container {' + "    width: ".concat(100 - firstColumnWidth, "%;") + '    float: right;' + '    display: inline-block;' + '    height: 100%;' + '}', '.chm-legend {' + '    padding-top: 17px;' + '    display: inline-block;' + '}', '.chm-legend > * {' + '}', '#chm-crf-legend {' + '    float: left;' + "    width: ".concat(12.5 * crfsCount, "%;") + '}', '#chm-query-legend {' + '    float: right;' + "    width: ".concat(12.5 * queriesCount, "%;") + '}', '.chm-legend-title {' + '    font-size: 20px;' + '    font-weight: bold;' + '}', '#chm-query-legend .chm-legend-title {' + '    text-align: right;' + '}', '.chm-legend-div {' + '    display: inline-block;' + '    height: 20px;' + '    text-align: center;' + '    font-weight: bold;' + '    font-size: 14px;' + '}', '#chm-crf-legend .chm-legend-div {' + '    width: 20%;' + '}', '#chm-query-legend .chm-legend-div {' + '    width: 20%;' + '}',
      /****---------------------------------------------------------------------------------\
        Table
      \---------------------------------------------------------------------------------****/
      '#chm-table {' + '    width: 100%;' + '}', '#chm-table table {' + '    display: table;' + '    width: 100%;' + '}', '.wc-table {' + '    display: block;' + '}', '.wc-table table thead tr th {' + '    cursor: default;' + '}', '.wc-table table thead tr th,' + '.wc-table table tbody tr td {' + "    padding-right: ".concat(paddingRight, "px;") + "    padding-left: ".concat(paddingLeft, "px;") + '}', '.wc-table table thead tr th:first-child,' + '.wc-table table tbody tr td:first-child {' + "    width: ".concat(firstColumnWidth, "% !important;") + '    text-align: left;' + '}', '.wc-table table thead tr:not(#column-controls) th:nth-child(n + 2),' + '.wc-table table tbody tr td:nth-child(n + 2) {' + "    width: ".concat(otherColumnWidth, "% !important;") + '    text-align: left;' + '}',
      /* range sliders */
      ...
      /* ID cells */
      '.chm-cell--id {' + '    background: white;' + '   text-overflow: ellipsis;' + '   white-space: nowrap;' + '   overflow: hidden;' + '   max-width: 0px;' + '}', '.chm-table-row--expandable .chm-cell--id {' + '    color: blue;' + '    cursor: pointer;' + '    text-decoration: underline;' + '}', '.chm-cell--id--level2 {' + '    text-indent: 1em;' + '}', '.chm-cell--id--level3 {' + '    text-indent: 2em;' + '}',
      /* heat cells */
      '.chm-cell--heat {' + '    text-align: right;' + '    font-size: 12px;' + '    border: 1px solid white;' + '}', '.chm-cell--heat--level6,' + '.chm-cell--heat--level7,' + '.chm-cell--heat--level8,' + '.chm-cell--heat--level1,' + '.chm-cell--heat--level2,' + '.chm-cell--heat--level3 {' + '    color: black;' + '}', '.chm-cell--heat--level9,' + '.chm-cell--heat--level10,' + '.chm-cell--heat--level11,' + '.chm-cell--heat--level4,' + '.chm-cell--heat--level5 {' + '    color: white;' + '}', '.chm-cell--heat--level1 {' + '    background: #edf8e9;' + '}', '.chm-cell--heat--level2 {' + '    background: #bae4b3;' + '}', '.chm-cell--heat--level3 {' + '    background: #74c476' + '}', '.chm-cell--heat--level4 {' + '    background: #31a354;' + '}', '.chm-cell--heat--level5 {' + '    background: #006d2c;' + '}', '.chm-cell--heat--level6 {' + '    background: #eff3ff;' + '}', '.chm-cell--heat--level7 {' + '    background: #bdd7e7;' + '}', '.chm-cell--heat--level8 {' + '    background: #6baed6' + '}', '.chm-cell--heat--level9 {' + '    background: #3182bd;' + '}', '.chm-cell--heat--level10 {' + '    background: #08519c;' + '}', '.chm-cell--heat--level11 {' + '    background: #08519c;' + '    color: white;' + '}']; //Attach styles to DOM.
    ...
    function syncSettings(settings) {
      ...
      if (settings.default_nesting.length > 3) {
        throw 'More than three default nesting columns were provided [' + settings.default_nesting.join(', ') + ']. Only three variables can be nested at a time. Please reduce the number of variables in the default_nesting setting.';
      ...
      settings.filter_cols = Array.isArray(settings.filter_cols) ? nest_settings.concat(settings.filter_cols) : nest_settings; //Define cols to include in subject level export
    ...
    function removeFilters() {
      ...
      this.controls.config.inputs = this.controls.config.inputs.filter(function (input) {
        if (input.type !== 'subsetter') {
          ...
          console.warn("The [ ".concat(input.label, " ] filter has been removed because the variable does not exist."));
        } else {
          var levels = d3.set(_this.data.raw.map(function (d) {
            return d[input.value_col];
          })).values();
          if (levels.length === 1) console.warn("The [ ".concat(input.label, " ] filter has been removed because the variable has only one level."));
    ...
    function removeSubjectExportCols() {
      ...
      if (export_cols.length > 0) {
        var subjectSetSize = d3.set(this.data.initial.map(function (d) {
          return d[_this.config.id_col];
        })).size();
        export_cols.forEach(function (col) {
          if (d3.set(context.data.initial.map(function (d) {
            return d[context.initial_config.id_col] + d[col];
          })).size() !== subjectSetSize) {
            console.warn("".concat(col, " was removed from the subject level export due to multiple values within subject."));
            context.initial_config.subject_export_cols.splice(context.initial_config.subject_export_cols.findIndex(function (d) {
        ...
        dropdown.on('change', function (di) {
          ...
          var loading = setInterval(function () {
            ...
            if (loadingIndicated) {
              ...
              context.filters.find(function (filter) {
                return filter.col === di.value_col;
              }).index = _this.multiple ? null : dropdown.selectAll('option:checked').property('index'); //Filter data.
    ...
    function customizeCells(chart, cells) {
      cells.attr('class', function (d) {
        ...
        return d.col === 'id' ? d.text.split('  |')[d.text.split('  |').length - 1] : chart.typeDict[d.col] == 'crfs' ? d.text === 'N/A' ? d.text : d3.format('%')(d.text) : d.text;
      });
    }
    ...
    function layout(filter) {
      ...
      filter.div = filter.cell.append('div').datum(filter).classed('range-slider-container', true); //lower slider
      ...
      filter.lowerAnnotation = filter.div.append('span').classed('range-annotation range-annotation--lower', true); //upper slider
    ...
    function layout$1(filter) {
      ...
      var rangeValueLowerDiv = filter.div.append('div').classed('range-value-container range-value-container--lower', true); //lower Input Box
      ...
      var rangeValueUpperDiv = filter.div.append('div').classed('range-value-container range-value-container--upper', true); //upper Input Box
    ...
    function formatControls() {
      ...
      this.controls.filters = {
        container: this.controls.wrap.insert('div').classed('chm-control-grouping chm-filters', true)
      };
      this.controls.filters.container.append('div').classed('chm-control-grouping--label', true).text('Filters');
      ...
      this.controls.otherControls = {
        container: this.controls.wrap.insert('div').classed('chm-control-grouping chm-other-controls', true)
      };
      ...
      this.controls.otherControls.controlGroups = this.controls.wrap.selectAll('.control-group:not(.chm-subsetter)');
    ...
    function deriveData() {
      ...
      this["export"].data.forEach(function (d, i, thisArray) {
        ...
        if ((_this.config.site_col || _this.config.subject_export_cols) && subject_id_col) {
          var subjectID = d["Nest ".concat(subject_id_col_index + 1, ": ").concat(_this.config.id_col)];
    ...
    function csv() {
      ...
      this["export"].data.forEach(function (d) {
        ...
        var row = _this["export"].cols.map(function (col, i) {
          var value = value_cols.indexOf(col) > -1 && context.typeDict[col] == 'crfs' && ['N/A', ''].indexOf(d[col]) < 0 ? d[col] * 100 : d[col];
      ...
      var fileName = "CRF-Heat-Map-".concat(d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()), ".csv");
    ...
    function xlsx() {
      ...
      workbook.Sheets[sheetName]['!autofilter'] = {
        ref: "A1:".concat(String.fromCharCode(64 + this["export"].cols.length)).concat(this["export"].data.length + 1)
      ...
      var fileName = "CRF-Summary-".concat(d3.time.format('%Y-%m-%dT%H-%M-%S')(new Date()), ".xlsx");
    ...
    function onDraw() {
      ...
      this.controls.wrap.selectAll('.control-group').filter(function (d) {
        return d.option === 'expand_all';
      }).select('.changer').property('checked', false); //end performance test
    ...
    function checkRequiredVariables() {
      ...
      if (missingVariables.length) console.log("The data are missing ".concat(missingVariables.length === 1 ? 'this variable' : 'these variables', ": ").concat(missingVariables.join(', '), "."));
    }
    ...
    function crfHeatMap(element, settings) {
      ...
      crfHeatMap.controls = webcharts.createControls(crfHeatMap.containers.controls.node(), crfHeatMap.settings.controls); //table
